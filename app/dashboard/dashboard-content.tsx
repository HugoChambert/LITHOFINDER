...
import { Plus, CreditCard as Edit2, Trash2, Building2, CreditCard } from 'lucide-react';
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {success && (
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-800 dark:text-green-200 px-4 py-3 rounded-lg mb-6">
          Subscription activated successfully! You can now upload slabs.
        </div>
      )}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600 dark:text-gray-400">
          ...
          <div>
            ...
            <span className={isActive ? 'text-green-600 dark:text-green-400 font-medium' : 'text-red-600 dark:text-red-400 font-medium'}>
              {supplier.subscription_status ? supplier.subscription_status.toUpperCase() : 'INACTIVE'}
            </span>
          </div>
        </div>
      </div>
      ...
      {!isActive && (
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 text-yellow-800 dark:text-yellow-200 px-6 py-4 rounded-lg mb-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-semibold mb-1">Subscription Required</p>
              <p className="text-sm">You need an active subscription to upload slabs.</p>
            </div>
            <Link
              href="/dashboard/subscription"
              className="bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors whitespace-nowrap"
            >
              Subscribe Now
            </Link>
          </div>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Link
          ...
          className={`font-semibold py-2.5 px-4 rounded-lg transition-colors flex items-center space-x-2 ${
            isActive
          ...
          onClick={(e) => {
            if (!isActive) {
              e.preventDefault();
              alert('You need an active subscription to add slabs');
            }
          }}
        >
